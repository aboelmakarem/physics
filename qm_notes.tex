...
%   \begin{itemize}
%     \item If the system's ket $\ket{\psi}$ is an eigenket of $S^n$, we say that the spin axis of the particle is $\hat{n}$
...
%   \end{itemize}
%   $S^n$ is assumed to linearly depend on $\hat{n}$ components
%     \begin{itemize}
%     \item $S^n = n_x A + n_y B + n_z C$ where $A,B$ and $C$ are Hermitian operators
%     \item For X-axis spin, $n_i = \delta_{1i} \implies S^x = A$, similarly, $S^y = B$ and $S^z = C$
%     \item Hence $S^n = n_x S^x + n_y S^y + n_z S^z = n_i S^i$
%   \end{itemize}
% \end{frame}
...
%\begin{frame}
  ...
  %\begin{itemize}
    ...
    %\item A time translation of $\alpha$ transforms $\ket{\psi(t)} \to \ket{\psi'(t)} = \exp({i \over \hbar} \alpha H) \ket{\psi(t)} \implies \ket{\psi'(t)} = \ket{\psi(t - \alpha)}$ 
    %\item Then $\exp({i \over \hbar} \alpha H) \ket{\psi(t)} = \ket{\psi(t - \alpha)}$, at $\alpha = t: $\exp({i \over \hbar} t H) \ket{\psi(t)} = \ket{\psi(0)}$
    %\item $\ket{\psi(t)} = \exp({-i \over \hbar} t H) \ket{\psi(0)} \implies {d \over dt} \ket{\psi(t)} = {-i \over \hbar} H \exp({-i \over \hbar} t H) \ket{\psi(0)} = {-i \over \hbar} H \ket{\psi(t)}$
    %\item $i \hbar {d \over dt} \ket{\psi(t)} = H \ket{\psi(t)}$ which is Schrodinger equation
    %\item The above derivation can be used in place of postulating that system's evolution follows Schrodinger's equation 
