...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Difference particles have, in general, different spin axes
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \vspace{1mm}
  The spin operator $S$ for a particle depends on its spin axis
  \begin{itemize}
    \item For spin axes in the $x,y$ and $z$ directions, the particle spin operators are $S_x,S_y$ and $S_z$ respectively
    \item $S_x,S_y$ and $S_z$ are Hermitian operators that depend on the particle's nature. Their forms will be determined later
    \item For a general spin axis in the $\hat{n} = (n_x,n_y,n_z)$ direction, the spin operator $S = n_x S_x + n_y S_y + n_z S_z$
    \item $S$ has $M$ eigenvalues equal to $s_1,s_2,...,s_M$, the $M$ spin DoF allowable values 
  \end{itemize}
  The particle does not spin about any axis, hence, the spin axis is only a conceptual direction that causes the combination $S = n_x S_x + n_y S_y + n_z S_z$ to give an operator with eigenvalues $\{s_1,s_2,...,s_M\}$
  \begin{itemize}
...
\begin{frame}
  \frametitle{Spinors}
  \vspace{1mm}
  The position operator kets $\ket{x_i}$ are members of a position Hilbert space $H_x$. Similarly, the spin operator kets $\ket{s_j}$ are members of a spin Hilbert space $H_s$
  \begin{itemize}
    ...
    \item The system kets $\psi$ are members of the product Hilbert space $H$
  ...
  \begin{itemize}
    ...
    \item $\ket{\psi}$ is the outer product of $H_x$ kets $\ket{\psi_j}$ and $H_s$ basis kets $\ket{s_j}$
  \end{itemize}
