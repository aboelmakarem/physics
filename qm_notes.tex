...
\begin{frame}
  ...
  \begin{itemize}
    \item $\ket{\psi} = \Sigma^{M_1}_{i_1 = 1} ... \Sigma^{M_N}_{i_N = 1} \int_{x_1} ... \int_{x_{3N}} \psi(x_1,...,x_{3N},s_{i_1},...,s_{i_N}) \ket{x_1 ... x_{3N} s_1 ... s_N} dx_1 ... dx_{3N}$
    \item Here, $M_1,M_2,...,M_N$ is the number of allowed spin values for particles $1,2,...,N$
    \item Let $M = M_1 \times M_2 \times ... \times M_N$, $\int_{\Omega} = \int_{x_1} \int_{x_2} ... \int_{x_{3N}}$ and $d\Omega = dx_1 dx_2 ... dx_{3N}$
    \item Hence $\ket{\psi} = \Sigma^{M}_{j = 1} \int_{\Omega} \psi(x_i,s_j) \ket{x_i,s_j} d\Omega$ where $x_i$ is a configuration in the $3N$-dimensional continuous position space and $s_j$ is a configuration in the $N$-dimensional discrete spin space
    \item $\psi(x_i,s_j)$ is called the system's wave function. It is a function in all of the system's degrees of freedom
    \item $\psi(x_i,s_j)$ is the projection of $\ket{\psi} on $\ket{x_i,s_j}$: $\braket{x_i,s_j}{\psi} = \Sigma^{M}_{m = 1} \int_{\Omega} \psi(x_l,s_m) \braket{x_i,s_j}{x_l,s_m} d\Omega = \psi(x_i,s_j)$
  \end{itemize}
...
\begin{frame}
  ...
  \vspace{1mm}
  Starting with $3$ Hermitian operators $S_x, S_y$ and $S_z$ and the components of a $3D$ unit vector $\hat{n}$ 
  \begin{itemize}
    ...
    \item For a single-particle system, if the system's ket $\psi$ is an eigenket of $S_n$, we say that the spin axis of the particle is $\hat{n}$
    \item Otherwise, the particle spins about some other axis which can be obtained by varying $n_x,n_y$ and $n_z$ until the particle's ket becomes an eigenket of $S_n$ 
    \item For multiple-particle systems, the spin operator is the tensor product of the particles spin operators with each of them generated by a different axis $\hat{n_i}$
    \item For spin axes in the $x,y$ and $z$ directions, the particle spin operators are $S_x,S_y$ and $S_z$ respectively
    \item For any $\hat{n}$, $S_n$ has $M$ eigenvalues equal to $s_1,s_2,...,s_M$, the $M$ spin DoF allowable values
    \item But as $\hat{n}$ changes, the eigenkets corresponding to the $M$ eigenvalues change 
...
\begin{frame}
  ...
  \vspace{1mm}
  A transformation to the space-time coordinates (STC) used to describe a physical system can not change the state of the system or the laws governing its evolution. Hence, the effect of an STC transformation can only be modeled by a symmetry transformation

  Nature is invariant under the following STC transformations.
  \begin{itemize}
...
\begin{frame}
  ...
  \vspace{1mm}
  Upon applying an STC transformation $\tau$, spinor component kets $\ket{\psi_k}$ transform to kets $\ket{\psi'_k} = U(\tau) \ket{\psi_k}$
  \begin{itemize}
    \item The kets $\ket{x_i}$ spanning $H_x$ transform to $\ket{x'_i}$ where $x'_i = \tau(x_i)$
    \item $\braket{x_i}{\psi_k} = \psi_k(x_i)$ and $\braket{x'_i}{\psi'_k} = \psi'_k(x'_i)$ 
    \item Note that $\psi(x_i) = \psi'_k(x'_i) = \psi'_k(\tau(x_i)) = \psi_k(\tau^{-1}(x'_i))$
    \item But $\ket{\psi'_k} = U(\tau) \ket{\psi_k} \implies \psi'_k(x'_i) = U(\tau) \psi_k(x'_i) = \psi_k(\tau^{-1}(x'_i))$
    \item In general, $U(\tau) \psi_k(x_i) = \psi_k(\tau^{-1}(x_i)$
    \item The kets transform in a direction opposite to the coordinate system transformation as expected to preserve their values. This is true because $U(\tau)$ is a symmetry transformation
  \end{itemize}
...
\begin{frame}
  ...
  \vspace{1mm}
  In general, the application of an STC transformation changes the spinors describing the system's state in two ways
  \begin{itemize}
    \item The components $\ket{\psi_k}$ of the spinors transform such that their values at one space-time point after transformation are reassigned to the values they had to the same space-time point before transformation $\psi'_k(x'_i,s'_j) = U(\tau) \psi_k(x_i,s_j) = \psi_k(\tau^{-1}(x_i,s_j))$
    \item This is required to preseve the particle's position DoFs and all dynamical variables depending on them after the transformation
    \item The components of the transformed spinor $\ket{\psi'}$ become linear combinations of the components of the spinor $\ket{\psi}$ before transformation
    \item This is required to preserve the particle's spin DoFs after the transformation 
  \end{itemize}
  Hence, the most general effect of an STC transformation on a spinor $\ket{\psi}$ is to take it to $\ket{\psi'}$ such that
  \begin{itemize}
    \item $\ket{\psi'} = U(\tau) D(\tau) \ket{\psi} = D(\tau) U(\tau) \ket{\psi}$
    \item $U(\tau)$ transforms the components $\ket{\psi_k}$ of $\ket{\psi}$ but does not form linear combinations of them
    \item $D(\tau)$ forms linear combinations of $\ket{\psi}$ components $\ket{\psi_k}$ but does not change each of them independently
    \item $U(\tau)$ and $D(\tau)$ commute because they act on $\ket{\psi}$ independently 
    \item $U(\tau)$ is a continuous transformation for the continuous components $\ket{\psi_k}$ while $D(\tau)$ is a discrete transformation for $\ket{\psi}$ components (an $M \otimes M$ matrix)
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item $P_i$ operates only on the positions of the $H$ spanning kets, the corresponding $H$ operator is $P_i \otimes I$
  ...
  \begin{itemize}
    ...
    \item The finite translation transformation is $U(s) = \exp(s {\partial \over \partial x_i})$
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \vspace{1mm}
  Unlike translation, upon a rotation by an infinitesimal angle $\epsilon$ in space about the $i$ axis: 
  \begin{itemize}
    \item $x_k \to x_k + \epsilon \epsilon_{ijk} x_i e^j$ where $e^j$ is the unit vector in $e^j$ direction ($e^j_p = \delta_{jp}$)
    \item $s_k \to s_k + ... $
  \end{itemize}
